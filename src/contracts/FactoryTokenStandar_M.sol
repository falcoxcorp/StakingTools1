...
contract FactoryBase is Context, Ownable {
    ...
    function addContractToken(address owner, string memory _name, string memory _symbol, address _tokenAddress) internal {
        myTokens[owner].push(TokenInfo(_name, _symbol, _tokenAddress));
    }
    
    function getContractTokenByAddress(address owner) external view returns (TokenInfo[] memory){
    ...
    function isERC20Token(address tokenAddress) internal view virtual returns (bool) {
    ...
    function requireValidERC20Token(address _tokenAddress) internal view virtual {
        require(
        ...
        onlyOwner
    {
        ...
        require(tokenContract.balanceOf(_msgSender()) >= _amount,"No hay suficientes tokens para retirar" );        
        require(tokenContract.transfer(owner(), _amount),"Error al retirar tokens");
...
contract FactoryTokenStandar is FactoryBase {
    ...
    function deployPaidBNB(
        ...
        require(msg.value == configBNB.paymentAmount,"No tienes suficiente saldo para pagar el servicio");
    ...
    function deployPaidByToken(
        ...
        IERC20 tokenContract = IERC20(_tokenAddress);
        require(tokenContract.balanceOf(_msgSender()) >= toPayment,"No tienes suficientes tokens" );      
        require(tokenContract.approve(_msgSender(), toPayment), "Error al aprobar tokens");
        require(tokenContract.allowance(_msgSender(), address(this)) >= toPayment,"Error al aprobar los tokens");
        require(tokenContract.transferFrom(_msgSender(), address(this), toPayment),"Error en transferencia de los tokens");
        address newERC20 = tokenFactory.createToken(
            _NAME,
            _SYMBOL,
            _DECIMALS,
