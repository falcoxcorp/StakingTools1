...
export const createTokenERC20Schema = Yup.object().shape({
  ...
  _txFee: Yup.number().when('activeStep', {
    is: STEP_ENUM.FACTORY,
    then: (schema) => schema.integer(YUP_ERRORS.ERROR_IS_INTEGER).required(YUP_ERRORS.REQUIRED).default(0).min(0, YUP_ERRORS.MIN_0).max(100, YUP_ERRORS.MAX_100).typeError('')
      .nullable()
  }),
