...
import { ICreateTokenERC20StandardExtender } from "../CreateTokenModule/hooks/ERC20/useERC20StepStandardForm"
import { getBigNumberSupply } from "../../utils/number"

export const useCreateTokenStandardPaidEther = () => {

  const { onVerifyNetworks, getContractAddress } = useUtilsChecking()
  const { sendTransaction, state, resetState } = useSendTransaction();

  const onPaidByToken = useCallback(async (createToken: ICreateTokenERC20StandardExtender) => {
  ...
  const onPaidByEther = useCallback(async (createToken: ICreateTokenERC20StandardExtender) => {
    const address = createToken?.network?.contractInstance?.address
  ...
  const onCreateTokenPaidEther = useCallback(async (createToken: ICreateTokenERC20StandardExtender) => {
