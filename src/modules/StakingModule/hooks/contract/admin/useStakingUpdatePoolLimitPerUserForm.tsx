...
export const useStakingUpdatePoolLimitPerUserForm = (stakingContract: string) => {
  const { updatePoolLimitPerUser, isLoading } = useStakingUpdatePoolLimitPerUser({
  ...
  return {
    ...
    onSubmit: handleSubmit((values: any) => {
      updatePoolLimitPerUser(values?.amount, values?.amount === 0 ? false : true)
    }),
    isLoading
  };
};

export const useStakingUpdatePoolLimitPerUser = ({ SPENDER_ADDRESS }: { SPENDER_ADDRESS: string }) => {
  ...
  useEffect(() => {
    if (state) {
      if (state.status === 'Exception' || state.status === 'Fail') {
        ...
        toast.success(t('message.updatePoolLimitPerUser'))
        queryClient.removeQueries([METHODS_STAKING_FACTORY.updatePoolLimitPerUser])
        resetState()
        return
      }
    }
  ...
  const updatePoolLimitPerUser = useCallback(async (amount: number, _userLimit: boolean) => {
    onToggle()
    await sendTransaction({
      to: SPENDER_ADDRESS,
      value: 0,
      data: instance.interface.encodeFunctionData(METHODS_STAKING_FACTORY.updatePoolLimitPerUser, [_userLimit, getParseEther(amount)])
