...
type StakingStepperActionsProps = {
  handleBack: any,
  activeStep: number
  chainId: number
  status: string
...
const StakingStepperActions = ({ handleBack, activeStep, chainId, status, isVerify, form = 'create-token-standard-form' }: StakingStepperActionsProps) => {
  const { t } = useTranslation(['staking', 'erc20'])
    ...
    <Stack gap={2}>
      ...
      <Stack gap={2} flexDirection={{ xs: 'column', md: 'row' }} >
        ...
        <Button
          ...
          type='submit'
          form={form}>
          {t('erc20:actions.next')}
        </Button>
        <Button
          fullWidth
          variant='contained'
          disabled={activeStep != 2 || loading.includes(status) || !account}
          sx={{ mr: 1 }}
          type='submit'
          form={form}>
          {t('staking:action.create')}
        </Button>
...
export default memo(StakingStepperActions);