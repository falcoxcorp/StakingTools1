...
const ConfirmStandard = ({ chainId, getValues, isLoading, status, nameToken, reset, paymentAmount }: ConfirmProps) => {
  const data = useMemo(() => getValues(), [getValues])
  const network = useMemo(() => LIST_NETWORKS_ERC20[TOKEN_TYPE_ENUM.STANDARD]?.find(nt => nt.chainId === chainId), [chainId])
            ...
            <>
              ...
              <PaperSection title={t('erc20:confirm:token:details')}>
                <Stack gap={1}>
                  ...
                  <StackItem title={t('form.routerAddress')} data={data?.routerAddress} />
...
export default memo(ConfirmStandard);